...
Packaged CLI update check and `locws upgrade`. Notices go to **stderr**, not the dashboard. The browser never sends a shell string — upgrade argv is hardcoded `npm install -g @y-corps/locws@latest`. A git clone uses `cloneHelpText` / `cloneUpgradeMessage` instead of locws usage.
...
| `upgradeArgv` | `npm` / `npm.cmd` + `install -g @y-corps/locws@latest` |
