...
| `fetchLatestVersion` | `GET https://registry.npmjs.org/locws/latest` (~3s timeout; `/` in the name is encoded). Failures return `""` |
...
[`test/cli/update-check.test.mjs`](../../../../test/cli/update-check.test.mjs) — semver, argv, skip/clone, clone vs packaged help, clone upgrade copy, prerelease vs same `x.y.z` latest, notice text, upgrade spawn (injected; no real global install, no live registry), registry path.
