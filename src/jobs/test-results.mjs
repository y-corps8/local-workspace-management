...
import { CACHE_DIR, LAST_TEST_RUNS_PATH, REPOS } from "../config/commands.mjs";
...
// ── Snapshot merge ─────────────────────────────────────────────────────────
// After an overview-launched test job, jobs.mjs writes a snapshot. If that
...
export function saveTestSnapshot(repoId, snapshot) {
  ...
  all[repoId] = snapshot;
  const tmp = path.join(CACHE_DIR, `.last-test-runs.${process.pid}.tmp`);
  fs.writeFileSync(tmp, `${JSON.stringify(all, null, 2)}\n`);
  fs.renameSync(tmp, LAST_TEST_RUNS_PATH);
}
