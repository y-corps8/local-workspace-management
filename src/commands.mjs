...
export function sanitizeRawWorkspace(raw) {
  ...
  const legacyBase = legacyRoot ? resolveUserPath(legacyRoot, APP_ROOT) : null;
...
function parseWorkspace(raw) {
  ...
  for (const project of clean.projects) {
    const root = resolveUserPath(project.path, APP_ROOT);
    repos[project.id] = {
      id: project.id,
      name: project.name,
...
function atomicWriteWorkspace(raw) {
  ...
  const tmp = path.join(CACHE_DIR, `.workspace.${process.pid}.tmp`);
...
export function startWorkspaceWatcher() {
  ...
  try {
    ...
    fs.watch(APP_ROOT, (_event, filename) => {
...
export function probeProject({ path: projectPath }) {
  const pathInput = String(projectPath ?? "").trim();
  if (!pathInput) {
    throw new Error("path is required");
  }
  const resolved = resolveUserPath(pathInput, APP_ROOT);
