...
- Dual-mode data: git clone keeps `workspace.json` and `.cache/` at `APP_ROOT`. Packaged installs (`node_modules` in `APP_ROOT`, e.g. npx / `npm i -g @y-corps/locws`) use `~/.config/locws/workspace.json` and `~/.cache/locws/` (Windows `%APPDATA%\locws` / `%LOCALAPPDATA%\locws`). `OVERVIEW_DATA_DIR` overrides both.
