...
function statusFromCounts({ failed, exitCode, success }) {
...
// ── Jest (bs-app, bs-admin, bs-app-db) ─────────────────────────────────────
...
/** Istanbul line % first; bs-app-db falls back to schema/behavioral summaries. */
...
// ── Maven Surefire / Failsafe (bs-spring) ──────────────────────────────────
...
function mergeReport(artifact, snapshot) {
...
export function readLastTestRun(repoId) {
  ...
  const snapshot = loadSnapshots()[repoId] ?? null;
...
export function readAllLastTestRuns() {
  ...
  for (const repoId of Object.keys(REPOS)) {
    result[repoId] = readLastTestRun(repoId);
  }
  return result;
}
